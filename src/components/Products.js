...
function Products(props) {
    ...
    const productInfo = {
        ...
        product5: [
            "한지방수매트",
            "닥펄프에 황토, 한지파우더(항산화물질)등을 혼합하여 생산한 메트로 식품용 에폭시 접착제를 제외한 어떠한 화학적 물질도 사용하지 않은 천연제품이다. 원적외선 방사율 99%, 대장균, 녹농균, 폐렴균에 의한 항균 99%,   탈취율 97%의 항균기능성 방수제품으로 의료병원 및 요양병원 등 시설의 환자 및 입원자의 침상 배트로 사용되는 제품입니다.",
        ],
        product6: [
            "한지파우더",
        ...
        <>
            {images.allFile.edges.map(({ node }, idx) => (
                <ProductCard
                    ...
                    key={idx}
                    name={productInfo[node.base.replace(".png", "")][0]}
                    info={productInfo[node.base.replace(".png", "")][1]}
                />
            ))}
        </>
    );
